...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            ...
            if place.bees and self.lower_bound <= space <= self.upper_bound:
                return random_bee(place.bees)
            place,space = place.entrance, space + 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        place_of_ant = self.place
        Ant.reduce_health(self,amount)
        # damage all bees
        bees = place_of_ant.bees
        copy= bees[:]
        bees = []
        for bee in copy:
        ...
        if not self.place:
            ...
            for bee in copy:
                ...
                if bee.health > 0:
                    bees.append(bee)
        print('DEBUG:',copy,bees)
        place_of_ant.bees = bees
...
# END Problem 7
def kill_insect(victim):
    Insect.reduce_health(victim, victim.health)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            place_ant = place.ant
            print('DEBUG:',place_ant,place)
            if place_ant is not None:
                if place_ant.is_container:
                    Ant.double(place_ant)
                    if place_ant.ant_contained is not None:
                        Ant.double(place_ant.ant_contained)
                else:
