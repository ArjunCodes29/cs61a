...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    player_value = player_score % 10
    opponent_value = (opponent_score // 10) % 10
    k =  3 * abs(player_value - opponent_value) 
    if k > 1:
        return k
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    i = 0
    k = 0
    test_value = strategy(0,0)
    while i < goal:
        k=0
        while k < goal:
            if not strategy(i,k)== test_value:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i = 1
    while i <= 10:
        a = make_averaged(roll_dice, total_samples)(i, dice)
        if a > current_highest:
...
def timid_strategy(score, opponent_score):
    return 2

def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.

    If we can win the game using boar brawl then we will do this
    if on averager boar brawling will give us more points then rolling 6 then we will do this
    IF we are just 3 away from our goal we will roll one dice
    if we are just 6 away from our goal we will roll 2 dice
    """
    # BEGIN PROBLEM 12
    if fuzzy_update(0, score, opponent_score) >= 100:
        return 0
    
    if fuzzy_update(0, score, opponent_score) - score > 11:
        return 0
    if score>96:
        return 1
    if score>93:
