...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(signature.first, do_lambda_form(Pair(formal,body),env))
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 5
    return expressions.first #quote is taken off anyways during scheme_apply
    # END PROBLEM 5
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # formals done , body list, return and create instance
    body = expressions.rest
...
def do_and_form(expressions, env):
    ...
    var = True
    while expressions:
        element = scheme_eval(expressions.first,env)

        if not element and (not type(element) in (float,int)):
            return element
        var = element
...
def do_or_form(expressions, env):
    ...
    while expressions:
        element = scheme_eval(expressions.first,env)
        if element or type(element) in (float,int):
            return element
        var = element
        expressions= expressions.rest
    return var
...
def make_let_frame(bindings, env):
    ...
    # decipher the names
    #decipher the values
    #validate there are the same number of names and values
    # validate that each name is distinct
