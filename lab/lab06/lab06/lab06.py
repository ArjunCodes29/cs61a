...
def prune_leaves(t, vals):
    ...
    new_branch= []
    for b in branches(t):
        if is_tree(prune_leaves(b,vals)):
...
def subseqs(s):
    ...
    rest = s[1:]
    return insert_into_all(s[0], subseqs(rest)) + subseqs(rest)
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return [[]] # none or something like this
        else:
            a = s[0]
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n==1 or n==2:
        ...
        b = [x for x in range(n-1,0,-1)]
        count = 0
        for i in range(len(a)):
            count = count + num_trees(a[i]) * num_trees(b[i])
        return count
...
def remove_odd_indices(lst, odd):
    ...
    if odd:
        return [lst[0]] + remove_odd_indices(lst[2:], odd)
         ...
         return [lst[1]] + remove_odd_indices(lst[2:], odd)      # not alternating the boolean in either call
...
def subset_sum(target, lst):
    ...
    if subset_sum(target-lst[0], lst[1:]):
        return True
    else:
        return subset_sum(target,lst[1:])
...
def trade(first, second):
    ...
    while not(m>len(first) or n>len(second)) and not equal_prefix():
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y)>0 or upper_bound(y)<0, 'Cant divide by 0'
...
def check_par():
    ...
    True
    """
    r1 = interval(21, 31)  # Replace this line!
    r2 = interval(3, 11)  # Replace this line!
    return r1, r2
...
def repeated(t, ko):
    ...
    def helper(prev,k,t):
        ...
        if a == prev:
            ...
            prev = a
            k = ko
        return helper(prev,k,t)
    return helper(None,ko,t)
