...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and':  # and expressions
            ...
            return calc_apply(OPERATORS.get(operator), operands)  # UPDATE THIS FOR Q2
...
def floor_div(expr):
    ...
    20
    """
    lst = expr.map(lambda x: calc_eval(x))
    result = lst.first * lst.first
...
def eval_and(operands):
    ...
    True
    """
    lst = operands.map(lambda x: calc_eval(x))
    while isinstance(lst, Pair):
        #print( "dinko", lst.first, not lst.first, not isinstance(lst.first,(float,int)))
        if not lst.first :
            if not type(lst.first) in (float,int):
                return lst.first
        if not isinstance(lst.rest, Pair):
            return lst.first
        lst = lst.rest
    return True
